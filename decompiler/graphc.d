...
{ /* Output code using flowgraph, interval and dominator details */
  ...
  nodeType* node=noddy;
  while((node->interval!=tempint)&&(node->interval!=nullinterval))
  ...
  if (dom!=node)
    switch(node->type)
    {
      case typeInt: /* Interval */
      {
        ...
        if((node->node.interval.latched)&&(node->node.interval.follow!=nullinterval))
        ...
        switch(node->node.dyad.oper)
        {
          case 'S':
          {
            switch(node->node.dyad.src1.type)
            {
              ...
              case 'v':
              {
                fprintf(out, "*%s",node->node.dyad.src1.var.name);
                break;
              }
              case 'l':
              {
          ...
          case 'L' :
          {
            ...
            switch(node->node.dyad.src1.type)
            {
              ...
              fprintf(out, "*r%d=(", node->node.dyad.dest.unas.value);
              traverse(regs[node->node.dyad.src1.unas.value]);
              fprintf(out, "+");
              ...
              case 'v':
              {
                ...
                if((temptree->object.oper.left->object.var=malloc(strlen(node->node.monad.src.var.name)+3*sizeof(char))) == NULL)
                  yyerror("out of memory");
                sprintf(regs[node->node.monad.dest.unas.value]->object.var, "*%s",node->node.dyad.src1.var.name);
          ...
          default: regs[node->node.dyad.dest.unas.value]=dodyad(&node->node.dyad.src1, &node->node.dyad.src2, node->node.dyad.oper);
        }
        codegen(node->next, dom, node->interval);
        break;
      }
      case typeCond:
      {
        ...
        if (node->next->type==typeMonad)
          switch (node->next->node.monad.oper)
          {
            ...
            case '%':
            {
              ...
              tmpcond->joint=node->node.cond.oper;
              tmpcond->next=cond;
              cond=tmpcond;
              cmp=1;
              break;
        ...
        if (!cmp)
        {
          int regmask=0;
          fprintf(out, "if (");
          condgen(cond, node->node.cond.oper);
          fprintf(out, ")\n{\n");
          ...
          {
            if ((node->node.cond.oper==next->node.cond.oper)&!invcond)
            {
              if (node->next->type==typeMonad)
                regmask|=(1<<node->node.monad.dest.unas.value);
              else if(node->next->type==typeDyad)
            ...
            else if(node->node.cond.oper==revcond(next->node.cond.oper))
            {
              if (!invcond)
                fprintf(out, "}\nelse\n{\n");
              invcond=1;
...
void condgen(condlist* conds, char cond)
{
  ...
  else if (conds->joint==revcond(cond))
...
void traverse(treeType* tree)
{
  switch (tree->type)
  {
    ...
    case typeLab :
    case typeVar :{fprintf(out, "%s", tree->object.var);break;}
    default: fprintf(out, "PANIC!\n");
  }
}
...
void ending(nodeType* nstart)
{
  ...
  istart->node=start;
  icurrent=istart;
  while(prevcount>intcount)
  {
    nodelist *inext;
    nodeType *nnext;
    printf("Next graph...\n");
    if (((inext = malloc(sizeof(nodelist))) == NULL)
       |((nnext = malloc(sizeof(nodeType))) == NULL))
      yyerror("Outta mem, guv!");
    printf("Allocated\n");
    nnext->type=typeInt;
    nnext->node.interval.header=icurrent->node;
    nnext->node.interval.latched=0;
    nnext->node.interval.succs=nullnode;
    nnext->node.interval.preds=nullnode;
    nnext->interval=nullinterval;
    inext->node=nnext;
    icurrent->node->interval=nnext;
    icurrent->next=inext;
    printf("Count=%d\n",prevcount=intcount);
    intcount=0;
    printf("Constructed\n");
